...
double test_network(Network network, double bounds){
    ...
    int counter = 0;
    for(int i = 0; i < 10; i++){
        x = -1*bounds+(static_cast<double>(std::rand())/RAND_MAX)*2*bounds;
        input[1] = x;
...
int main(int argc, char* argv[]){
    ...
    for(int i = 0; i < n_generations; i++){
        ...
        std::sort(pop_outputs.begin(), pop_outputs.end(), [](const auto& lhs, const auto& rhs) {
            return lhs.second > rhs.second;});
        improvement = (pop_outputs.at(0).second-best_prev);// improvement *= improvement;
        best_prev = pop_outputs.at(0).second;
        std::cout<<"\nbest dessa geracao: "<<pop_outputs.at(0).second<<" improvement: "<<improvement<<"\n";
        ...
        next_gen[0].copyNetwork(reproduce(population[pop_outputs.at(0).first], population[pop_outputs.at(1).first], chromosome, mode, true, rand(), mutation_range, mutation_chance, NULL));
        int p1, p2;
        for(int j = 1; j < cut; j++){
            p1 = rand()%cut; p2 = rand()%cut;
            printf("cruzando %d com %d\n", p1, p2);
            next_gen[j].copyNetwork(reproduce(population[pop_outputs.at(p1).first], population[pop_outputs.at(p2).first], chromosome, mode, true, rand(), mutation_range, mutation_chance, NULL));
        }
        for(int j = 0; j < cut; j++)
