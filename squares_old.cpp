...
private:
    ...
    Network*decision;
public:
    Bichinho(double calories, double attack, double defense, double speed, double x, double y);
    Bichinho();
    double getCalories();
    double getAttack();
    double getDefense();
...
Bichinho::Bichinho(double calories, double attack, double defense, double speed, double x, double y)
{
...
int main(int argc, char*argv[]){
    ...
    animais = new Bichinho[population_size];//(Bichinho*) malloc(population_size*sizeof(Bichinho));
    std::srand(getCurrentTimeInSeconds());
    for(int i = 0; i < population_size; i++){
        animais[i] = Bichinho(starting_calories, 1.0/3, 1.0/3, 1.0/3, boundedRand(rand(), -1, 1), boundedRand(rand(), -1, 1));
    }
    food_amount = 20;
    food = (Bichinho*) malloc(food_amount*sizeof(Bichinho));
    for(int i = 0; i < food_amount; i++){
        food[i] = Bichinho(food_calories, 0, 1, 0, boundedRand(rand(), -1, 1), boundedRand(rand(), -1, 1));
    }
    food_calories = 2;
...
void draw_bichinho(Bichinho bichinho){
    double x = bichinho.getx(), y = bichinho.gety(), len = bichinho.getLen(), calories = bichinho.getCalories();
    double red = bichinho.getAttack(), green = bichinho.getDefense(), blue = bichinho.getSpeed();
    std::cout<<"desenhando um bichinho em "<<x<<", "<<y<<" red: "<<red<<" green "<<green<<" blue "<<blue<<"\n";
